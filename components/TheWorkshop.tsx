...
const Workshop: React.FC = () => {
  ...
  const handleTextChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      if (foundSuggestions.length > 0) {
          // Calculate approx position (naive approach for textarea)
          // In a real app we'd use a proper overlay library or getBoundingClientRect on a dummy span
          // For now, fixed position near cursor is hard, so we put it top-left relative to textarea or just floating
  ...
  const insertSuggestion = (text: string) => {
      ...
      const textBefore = codeContent.slice(0, cursorIndex);
      const textAfter = codeContent.slice(cursorIndex);
      
      // If triggered by dot, just append
      // If triggered by word, replace word? Keep simple append for now.
      setCodeContent(textBefore + text + textAfter);
      setShowIntelliSense(false);
                  ...
                  {selectedFile?.fileType === 'script' ? (
                      viewMode === 'code' ? (
                          <div className="flex-1 relative h-full flex flex-col">
                            {/* IntelliSense Popup */}
                            {showIntelliSense && (
                                <div className="absolute z-20 bg-[#252526] border border-slate-600 rounded shadow-xl min-w-[200px] animate-fade-in" style={{ top: '60px', left: '60px' }}>
                                    <div className="text-[10px] bg-[#333] px-2 py-1 text-slate-400 font-bold flex items-center gap-1">
                                        <Lightbulb className="w-3 h-3 text-yellow-400" /> Suggestion
                                    </div>
                                    <div className="flex flex-col">
                                        {suggestions.map((s, i) => (
                                            <button 
                                                ...
                                                className="px-2 py-1 text-xs text-left text-slate-300 hover:bg-[#094771] hover:text-white flex items-center gap-2 font-mono"
                                            >
                                                <span className="text-purple-400 font-bold">f</span> {s}
                                            </button>
                                        ))}
                                    </div>
                                </div>
